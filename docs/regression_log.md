...
| R025 | 2026-05-27 | 2026-05-27 | "Bullseye minigame had no results" — NOT a separate bug; downstream of R020. With the aim D-pad dead (or in fire-only variants), every fire defaulted to quadrant A; unless the target was A, all fires missed → 0 points → no winner/bonus awarded → results panel shows only "no bonus". Verified the chain is resolved: aiming at the real target now yields 🥇+500/🥈+200. NOTE: VariantA/C remain fire-only (no aim UI) by design — bullseye is only fully winnable in VariantB (the game-controller layout the user plays). | bullseye-aim-produces-a-winner (verify_bullseye_scoring.py) — see [[R020]] |
