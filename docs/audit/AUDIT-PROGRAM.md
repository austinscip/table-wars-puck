...
| Runtime games — crash-recovery/race/reconnect (`server/runtime/*` + `server/games/*`) | ✅ done (2026-06-06) | `runtime-games-2026-06-06.md`. Dedicated pass for EVERY game: mid-match reconnect (was a strand gap like legacy), deserialize schema-skew tolerance, racer empty-finalize guard, Redis NaN-reject + atomic save, SP answer normalization, match-duration backstop, dt clamp. Time-rebase confirmed already correct. Lock/dict races re-graded N/A (greenlet, not preemptive). |
