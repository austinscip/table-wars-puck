...
1. **top_shell_petg.stl** - Translucent Blue PETG
2. **bottom_shell_petg.stl** - Black PETG
3. **button_cap_petg.stl** - Translucent Blue PETG

### TPU Parts (Black, Flexible)
4. **bumper_ring_tpu.stl** - Black TPU (Shore 95A)
5. **motor_sock_tpu.stl** - Black TPU (Shore 95A)
...
1. **Translucent PETG**: Must be translucent (not transparent) for LED diffusion
2. **Wall Thickness**: 3mm is critical - don't scale parts
3. **Button Position**: Button cap MUST protrude above dome (verified at z=44mm in assembly)
4. **PCB Clearance**: Custom PCB design with centered MPU6050 for precision motion sensing
5. **Support Removal**: Take care removing supports from dome interior and energy rings
6. **Post-Processing**: Light sanding on seam surfaces for smooth fit

## Questions?

Contact for any clarification on dimensions, materials, or assembly.
